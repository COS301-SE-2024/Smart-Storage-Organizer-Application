...
% include performance, reliability, scalability, security maintainability, usability.
    ...
    \subsection{Testing}
    \begin{itemize}
% WHAT ARE WE USING FOR OUR TESTING ???
...
% \subsection{Q6. Availability}
% Certain app features must remain accessible without an internet connection to ensure continuous usability.
% Implement local data storage using technologies like SQLite or IndexedDB for web applications.
% Use service workers to cache essential resources and enable offline access to previously loaded content.
% Synchronize data with the server once the connection is reestablished to ensure data consistency.

% \section{Trace-ability Matrix}
% Requirements VS Sub-systems
% A matrix with requirements numbers as rows and the sub-systems as columns. Include both functional and quality
% requirements. Indicate with x in the cells which subsystem will implement the solution to satisfy the requirement.
...
%     \hline
%                                      \multicolumn{7}{|c|}{Subsystems} \\ \hline
%                                     && S1 & S2  & S3 & S4 & S5  \\ \hline
%             \multirow{17}{*}{Requirements}  & R1 &X & & & &                                      \\\cline{2-7}
%                                             & R2 & &X & & &                    \\ \cline{2-7}
%                                             & R3 & & &X & &                      \\\cline{2-7}
%                                             & R4 & & & &X &                      \\\cline{2-7}
%                                             & R5 & &X & & &                      \\\cline{2-7}
...
%                                         \hline
%             \end{tabular}
%         \caption{Traceability matrix for Document Workflow}
%         \label{table:SystemTraceability}
%         \end{table}
