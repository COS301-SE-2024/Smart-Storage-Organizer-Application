...
def get_all_units(conn,curr,body):
    ...
    print("names",names)
    print("subcategory_ids_placeholder",subcategory_ids_placeholder)
    query = f"""
    SELECT
        COUNT(*) AS total_units,
        {', '.join([f"COUNT(CASE WHEN subcategoryid = %s THEN 1 END) AS subcategory_{i+1}_count" for i in range(len(subcategory_ids_placeholder))])}
    FROM items
    WHERE subcategoryid IN ({subcategory_ids_placeholder}) AND organizationid = %s;
    """
    params = (*ids, body['organizationid'])
