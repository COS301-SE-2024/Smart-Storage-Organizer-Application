...
def getSubCategories(conn,curr,organizationid,parentcategoryIds):
    query = "SELECT id, categoryname FROM category WHERE parentcategory IN %s AND (organizationid = %s  OR organizationid = 0)"
    # Execute the query
    curr.execute(query, (tuple(parentcategoryIds),organizationid))
    # Fetch the results
    results = curr.fetchall()
    return json.dumps(results)

def getSubCategoriesTotals(conn, curr,subcategories):
...
def lambda_handler(event, context):
    ...
    try:
        ...
        subCategories=getSubCategories(conn,curr,organizationid,ids)
        print(getSubCategoriesTotals(conn, curr, json.loads(subCategories)))
