...
def lambda_handler(event,context):
    ...
    if 'changes' not in body or 'changes_date_and_time' not in body or 'changed_by' not in body or 'related_record_id' not in body or 'related_record_type' not in body or 'related_record_name' not in body or 'changes_type' not in body or 'Reason_for_change' not in body or 'organization_id' not in body or 'previous' not in body or 'current' not in body:
        return {
            'statusCode': 400,
    ...
    try:
        query='INSERT INTO changes_made (change_description,change_date,changed_by,related_record_id,related_record_type,related_record_name,change_type,reason_for_change,organization_id,previous,current,comment) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        cur.execute(query,(body['changes'],body['changes_date_and_time'],body['changed_by'],body['related_record_id'],body['related_record_type'],body['related_record_name'],body['changes_type'],body['Reason_for_change'],body['organization_id'], json.dumps(body['previous']), json.dumps(body['current'])))
        con.commit()
