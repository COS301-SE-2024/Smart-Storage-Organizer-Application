...
def get_all_units(conn,curr,body):
    ...
    query = f"""
    SELECT id, categoryname FROM category WHERE organizationid =%s AND parentcategory = {body['parentcategory']}
    """
...
event={'body': {'organizationid': 1, 'subcategory': [{'id': 57, 'name': 'electronics'}, {'id': 59, 'name': 'subcategory2'}]}}
