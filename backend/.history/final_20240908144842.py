...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(list.__sizeof__(ids))
        print(list.__sizeof__(names))
        finalList=[]
        for i,j in zip(ids, names):
            ...
            # print(subsId)
            # print(subsName)
            # print(getSubCategoriesTotals(conn, curr,subsId,subsName,names[i]))
            print(names[j])
            finalList.append(getSubCategoriesTotals(conn, curr,subsId,subsName,names[j]))
